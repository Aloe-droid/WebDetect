function process_output_pose(output, size, image_width, image_height) {
    let boxes = [];
    let poses = [];
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        for (let j = 0; j < 51; j++) {
            if (j % 3 == 0) {
                keys.push((output[maxLen * (5 + j) + i]) * dx);
            } else if (j % 3 == 1) {
                keys.push((output[maxLen * (5 + j) + i]) * dy);
            } else {
                keys.push((output[maxLen * (5 + j) + i]));
            }
        }
        keys.push(i);
